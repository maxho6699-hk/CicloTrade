...
def _manifest(dataset: FrozenDailyOhlcv, source: bytes, snapshot: bytes, *, template_key: str = "equity.trend.long_flat.v1", minimum_coverage_days: int, minimum_trades: int = 30) -> dict[str, Any]:
    return {
        ...
        "search_space": {"lookback": [5, 10]}, "experiment_budget": {"runs": 2, "folds": 3}, "evidence_hashes": {"source.csv": hashlib.sha256(source).hexdigest(), "source-snapshot.json": hashlib.sha256(snapshot).hexdigest(), "prices.csv": dataset.sha256},
