...
;(async () => {
  ...
  for (const [plan, tier, file] of tiers) {
    ...
    const assetUrl = new URL(state.src, BASE)
    assert(assetUrl.origin === new URL(BASE).origin, `${plan}: cross-origin robot asset ${state.src}`)
    assert(assetUrl.pathname === `/assets/robot/${file}`, `${plan}: wrong asset ${state.src}`)
    assert(assetUrl.search === '?v=institutional-v3', `${plan}: unexpected robot asset query ${state.src}`)
  ...
  const deliberationAssetUrl = new URL(deliberationState.src, BASE)
  assert(deliberationAssetUrl.origin === new URL(BASE).origin, `deliberation: cross-origin asset ${deliberationState.src}`)
  assert(deliberationAssetUrl.pathname === '/assets/robot/robot-lv4.png', `deliberation: wrong asset ${deliberationState.src}`)
  assert(deliberationAssetUrl.search === '?v=institutional-v3', `deliberation: unexpected asset query ${deliberationState.src}`)
