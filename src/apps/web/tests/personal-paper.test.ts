...
test('personal paper risk proof is fail-closed for every decision branch', () => {
  ...
  assert.equal(validPersonalPaperRiskProof({ ...proof, decision: 'reject', risk_level: 'blocked', checks: failedChecks }), true)
...
test('personal paper risk check formatter rejects malformed JSON without exposing raw payloads', () => {
  ...
  assert.doesNotMatch(formatPersonalPaperRiskCheck({ ...checks[5], value: JSON.stringify({ scheduled_at: '2026-08-14T00:00:00Z', revision_id: 'evt_1', payload_sha256: 'a'.repeat(64) }), limit: JSON.stringify({ scheduled_at: 'must_be_known' }) }).value, /\{"/)
})
...
test('personal paper risk check formatter handles all seven fixed schemas', () => {
  const values = [
    ...
    { code: 'event_gap', value: { scheduled_at: '2026-08-14T00:00:00Z', revision_id: 'evt_1', payload_sha256: 'a'.repeat(64) }, limit: { scheduled_at: 'must_be_known' } },
