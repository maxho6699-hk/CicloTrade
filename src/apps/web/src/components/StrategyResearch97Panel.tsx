import { AlertTriangle, ChevronLeft, ChevronRight, Clock3, Database, FlaskConical, LockKeyhole, Search, ShieldAlert, ShieldCheck, SlidersHorizontal } from 'lucide-react'
...
interface ResearchCopy {
  title: string; subtitle: string; disclaimer: string; loading: string; error: string; forbidden: string; partial: string; mismatch: string; empty: string; waiting: string
...
export function StrategyResearch97Panel() {
  ...
  useEffect(() => { setPage(1) }, [query, tier, signal])
  ...
  const cycle = load.data?.latest.cycle ?? null
  ...
  return <section className="strategy-research-97-panel">
    <article className="data-panel strategy-research-97-summary">
      ...
      <p className="strategy-research-97-disclaimer"><ShieldAlert aria-hidden="true" size={16} />{text.disclaimer}</p>
    ...
    {partial ? <article className="data-panel"><div className="strategy-research-97-message" role="status" aria-live="polite"><Database aria-hidden="true" size={20} /><span>{latest?.cycle ? text.partial : text.error}</span></div></article>
      : statusData?.state === 'waiting' ? <article className="data-panel"><div className="strategy-research-97-message" role="status" aria-live="polite"><Clock3 aria-hidden="true" size={20} /><span>{text.waiting}</span></div></article>
        : !cycle ? <article className="data-panel"><div className="strategy-research-97-message" role="status" aria-live="polite"><FlaskConical aria-hidden="true" size={20} /><span>{text.empty}</span></div></article>
          : <>
            <article className="data-panel strategy-research-97-cycle">
              ...
              <div className="strategy-research-97-filters" aria-label={locale === 'zh-Hant' ? '研究篩選' : '研究筛选'}><label><span>{text.search}</span><div><Search aria-hidden="true" size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={text.searchPlaceholder} aria-label={text.search} /></div></label><label><span>{text.tierFilter}</span><select value={tier} onChange={(event) => setTier(event.target.value as TierFilter)} aria-label={text.tierFilter}><option value="all">{text.all}</option><option value="A">{text.tierA}</option><option value="C">{text.tierC}</option></select></label><label><span>{text.signalFilter}</span><select value={signal} onChange={(event) => setSignal(event.target.value as SignalFilter)} aria-label={text.signalFilter}><option value="all">{text.allSignals}</option><option value="long">{text.long}</option><option value="flat">{text.flat}</option><option value="wait">{text.wait}</option><option value="stale">{text.stale}</option><option value="missing">{text.missing}</option></select></label></div>
