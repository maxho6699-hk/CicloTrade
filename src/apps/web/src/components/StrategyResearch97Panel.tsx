...
type ResearchFilter = 'all' | StrategyResearch97Signal | StrategyResearch97DataState
...
interface ResearchCopy {
  ...
  stable: string; stableDetail: string; expanded: string; expandedDetail: string; unavailable: string; state: string; version: string; universe: string; coverage: string; updated: string; expires: string; cycle: string; strategy: string; wait: string; missing: string; fresh: string; stale: string; dataMissing: string; symbol: string; tier: string; dataState: string; signal: string; rationale: string; history: string; received: string; audit: string; active: string; invalidated: string; expired: string; superseded: string; noHistory: string; nonExecutable: string; search: string; searchPlaceholder: string; tierFilter: string; statusFilter: string; all: string; tierA: string; tierC: string; allStatuses: string; page: string; previous: string; next: string; resultCount: string; status: Record<StrategyResearch97State, string>
}

const SIMPLIFIED: ResearchCopy = {
  title: '97 只股票扩容研究', subtitle: '独立研究链 · 不覆盖 13 股稳定影子链', disclaimer: '本页只展示策略服务器回传的研究证据。研究结果不可执行、不可下单、不可推送 Telegram，也不会改变官方或实盘状态。', projection: '登录后的受控研究投影 · 原始策略源不可见', loading: '正在读取扩容研究记录。', error: '扩容研究暂时无法读取；为了安全，本页不会显示不完整或未经验证的结果。', forbidden: '当前账户没有读取扩容研究的权限。研究数据不会以演示数据替代。', partial: '部分研究资源暂时不可用；已读取的数据仍保持原始状态，不会被当作完整结果。', mismatch: '研究资源的版本或证据绑定不一致；为了安全，本页不会展示这次周期。', empty: '尚未收到可展示的扩容研究周期。', noMatches: '没有符合当前筛选条件的股票；请清除筛选后再试。', waiting: '扩容研究链正在等待下一次周期；不会生成可执行操作。', stable: '13 股稳定 shadow', stableDetail: '独立链 · 稳定覆盖', expanded: '97 只股票 research', expandedDetail: '本页 · 扩容观察', unavailable: '暂不可用', state: '运行状态', version: '研究版本', universe: 'Universe', coverage: '本周期覆盖', updated: '最后更新', expires: '证据过期时间', cycle: '最新周期', strategy: '策略摘要', wait: 'WAIT', missing: '无数据', fresh: '新鲜', stale: '已过期', dataMissing: '缺数据', symbol: '股票', tier: '层级', dataState: '数据状态', signal: '研究标签', rationale: '摘要', history: '最近研究周期', received: '收到时间', audit: '审计状态', active: '有效', invalidated: '已失效', expired: '已过期', superseded: '已替代', noHistory: '暂无历史周期。', nonExecutable: '仅供研究 · 不可操作', search: '搜索股票', searchPlaceholder: '输入股票代码…', tierFilter: '层级筛选', statusFilter: '状态筛选', all: '全部', tierA: 'Tier A · 稳定轮转', tierC: 'Tier C · 扩容观察', allStatuses: '全部状态', page: '页', previous: '上一页', next: '下一页', resultCount: '个结果', status: { waiting: '等待中', healthy: '正常', stale: '已过期', degraded: '降级' },
}

const TRADITIONAL: ResearchCopy = {
  title: '97 隻股票擴容研究', subtitle: '獨立研究鏈 · 不覆蓋 13 股穩定影子鏈', disclaimer: '本頁只展示策略伺服器回傳的研究證據。研究結果不可執行、不可下單、不可推送 Telegram，也不會改變官方或實盤狀態。', projection: '登入後的受控研究投影 · 原始策略源不可見', loading: '正在讀取擴容研究記錄。', error: '擴容研究暫時無法讀取；為了安全，本頁不會顯示不完整或未驗證的結果。', forbidden: '當前帳戶沒有讀取擴容研究的權限。研究資料不會以演示資料替代。', partial: '部分研究資源暫時不可用；已讀取的資料仍保持原始狀態，不會被當作完整結果。', mismatch: '研究資源的版本或證據綁定不一致；為了安全，本頁不會展示這次週期。', empty: '尚未收到可展示的擴容研究週期。', noMatches: '沒有符合目前篩選條件的股票；請清除篩選後再試。', waiting: '擴容研究鏈正在等待下一次週期；不會產生可執行操作。', stable: '13 股穩定 shadow', stableDetail: '獨立鏈 · 穩定覆蓋', expanded: '97 隻股票 research', expandedDetail: '本頁 · 擴容觀察', unavailable: '暫不可用', state: '運行狀態', version: '研究版本', universe: 'Universe', coverage: '本週期覆蓋', updated: '最後更新', expires: '證據過期時間', cycle: '最新週期', strategy: '策略摘要', wait: 'WAIT', missing: '無資料', fresh: '新鮮', stale: '已過期', dataMissing: '缺資料', symbol: '股票', tier: '層級', dataState: '資料狀態', signal: '研究標籤', rationale: '摘要', history: '最近研究週期', received: '收到時間', audit: '稽核狀態', active: '有效', invalidated: '已失效', expired: '已過期', superseded: '已替代', noHistory: '暫無歷史週期。', nonExecutable: '僅供研究 · 不可操作', search: '搜尋股票', searchPlaceholder: '輸入股票代碼…', tierFilter: '層級篩選', statusFilter: '狀態篩選', all: '全部', tierA: 'Tier A · 穩定輪轉', tierC: 'Tier C · 擴容觀察', allStatuses: '全部狀態', page: '頁', previous: '上一頁', next: '下一頁', resultCount: '個結果', status: { waiting: '等待中', healthy: '正常', stale: '已過期', degraded: '降級' },
}

const PAGE_SIZE = 18
const NARROW_PAGE_SIZE = 8
// Keep the existing URL key so saved links remain valid; its accepted values
// are now limited to WAIT/data states and never include directional labels.
...
function parseStatus(value: string | null): ResearchFilter {
  return value === 'wait' || value === 'stale' || value === 'missing' ? value : 'all'
}
...
function pageCountForLoad(loadState: LoadState, query: string, tier: TierFilter, status: ResearchFilter, pageSize: number): number {
...
export function StrategyResearch97Panel() {
  ...
  const [status, setStatus] = useState<ResearchFilter>(() => parseStatus(searchParams.get(QUERY_KEYS.signal)))
  ...
  const [isNarrowViewport, setIsNarrowViewport] = useState(() => typeof window !== 'undefined' && window.matchMedia('(max-width: 390px)').matches)
  ...
  useEffect(() => {
    ...
    setStatus(parseStatus(searchParams.get(QUERY_KEYS.signal)))
    setQuery(searchParams.get(QUERY_KEYS.query) ?? '')
    setPage(parsePage(searchParams.get(QUERY_KEYS.page)))
  }, [searchParams])

  useEffect(() => {
    const pageCount = pageCountForLoad(loadState, query, tier, status, isNarrowViewport ? NARROW_PAGE_SIZE : PAGE_SIZE)
    const safePage = Math.min(page, pageCount)
    if (page === safePage) return
    const next = new URLSearchParams(searchParams)
    if (safePage > 1) next.set(QUERY_KEYS.page, String(safePage))
    else next.delete(QUERY_KEYS.page)
    setPage(safePage)
    setSearchParams(next, { replace: true })
  }, [isNarrowViewport, loadState, page, query, searchParams, setSearchParams, status, tier])
  ...
  const safePage = Math.min(page, pageCount)
  const visibleSymbols = filteredSymbols.slice((safePage - 1) * pageSize, safePage * pageSize)
  ...
  const changeStatus = (next: ResearchFilter) => { setStatus(next); setPage(1); updateUrl({ [QUERY_KEYS.signal]: next === 'all' ? undefined : next, [QUERY_KEYS.page]: undefined }) }
    ...
    {!cycle ? <article className="data-panel"><div className="strategy-research-97-message" role="status" aria-live="polite"><Database aria-hidden="true" size={20} /><span>{partial ? text.partial : statusData?.state === 'waiting' ? text.waiting : text.empty}</span></div></article>
          : <>
            <article className="data-panel strategy-research-97-cycle">
              ...
              <div className="strategy-research-97-summary-grid"><div><span>{text.strategy}</span><strong>{cycle.strategy_name}</strong></div><div><span>{text.coverage}</span><strong>{cycle.summary.wait_count}/{cycle.summary.wait_count + cycle.summary.no_data_count}</strong></div><div><span>{text.wait}</span><strong className="warning-text">{cycle.summary.wait_count}</strong></div><div><span>{text.missing}</span><strong>{cycle.summary.no_data_count}</strong></div></div>
