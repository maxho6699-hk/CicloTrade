...
export function MarketsPage() {
  ...
  const inspectorDecision: Decision = officialDecision ?? {
    state: 'wait',
    action: 'wait',
    instrument: selected,
    title: '暂无当前正式行动',
    summary: '该股票当前没有有效的正式量化事件。系统不会根据界面行情临时拼出购买建议。',
    entry: '未生成',
    stop: '未生成',
    target: '未生成',
    maxLoss: '未生成',
    ...
    confidence: '未核验',
    evidence: [],
    counterEvidence: ['没有可验证的正式事件'],
    modelVersion: '无正式版本',
    eventId: 'NO-ACTIVE-EVENT',
    actionable: false,
    currentInstruction: '现在不买、不卖；数量 0',
    currentPrice: '未核对',
  ...
  const auxiliaryInspector = (
    <div className="canonical-auxiliary-inspector">
      ...
      {inspectorTab === '建议' && <div className="inspector-panel">
        <div className="inspector-heading"><span>OFFICIAL ACTION</span><strong>{officialDecision ? '正式行动合同' : '等待正式行动'}</strong></div>
        ...
        <h2>{officialDecision ? officialDecision.currentInstruction : '现在不买、不卖；数量 0'}</h2>
        <p>{inspectorDecision.summary}</p>
        <dl className="inspector-metrics">
          <div><dt>当前报价</dt><dd>{officialDecision?.currentPrice ?? '未核对'}</dd></div>
      ...
      <section className="chart-workspace">
        ...
        <div className="mobile-evidence-panel" id="mobile-evidence-panel">
          <nav className="workspace-tabs">{evidenceTabs.map((tab) => <button className={activeTab === tab ? 'active' : ''} type="button" onClick={() => setParam('tab', tab)} key={tab}>{tab}</button>)}{activeTab === '期权证据' && <button className="button secondary options-tab-cta" type="button" onClick={() => navigate('/lab')}>打开期权研究入口</button>}</nav>
           <EvidencePanel tab={activeTab} candles={chartData} decision={officialDecision} source={currentLiveCandles.length ? chartStatus : '暂无可验证行情'} />
        </div>
      </section>
