  ...
  return (
    <div className={`market-workspace research-market-workspace ${stockSwitcherOpen ? 'stock-switcher-open' : ''} ${mobileEvidenceOpen ? 'mobile-evidence-open' : ''}`}>
      <header className="instrument-header research-instrument-header">
        <div className="research-instrument-identity"><StockLogo symbol={selected.symbol} size="lg" /><span><strong>{selected.name}</strong><small>{selected.symbol} · {selected.market === 'US' ? '美股' : 'A股'} · {selected.currency}</small></span></div>
      ...
      {stockSwitcherOpen && <section className="research-stock-switcher" id="research-stock-switcher" aria-label="当前股票切换"><header><strong>切换当前股票</strong><span>不复制全局搜索或发现筛选</span></header><div className="market-tabs"><button className={marketFilter === 'US' ? 'active' : ''} type="button" onClick={() => setMarket('US')}>美股</button><button className={marketFilter === 'CN' ? 'active' : ''} type="button" onClick={() => setMarket('CN')}>A股</button></div><div className="research-switcher-list">{visibleInstruments.slice(0, 8).map((instrument) => <button className={instrument.symbol === selected.symbol ? 'active' : ''} type="button" onClick={() => openInstrument(instrument)} key={instrument.symbol}><StockLogo symbol={instrument.symbol} size="sm" /><span><strong>{instrument.symbol}</strong><small>{instrument.name}</small></span><span>{instrument.price ? instrument.price.toFixed(2) : '等待报价'}</span></button>)}{!visibleInstruments.length && <ResearchEmptyState compact title="当前市场没有自选股票" detail="前往发现页建立候选后会显示在这里。" />}</div>{switcherStatus && <p className="form-status" role="status" aria-live="polite">{switcherStatus}</p>}<button className="button tertiary wide" type="button" onClick={() => navigate('/discover')}>前往发现股票</button></section>}
