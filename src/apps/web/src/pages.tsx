  ...
  return (
    <div className={`market-workspace research-market-workspace ${stockSwitcherOpen ? 'stock-switcher-open' : ''} ${mobileEvidenceOpen ? 'mobile-evidence-open' : ''}`}>
      <header className="instrument-header research-instrument-header">
        <div className="research-instrument-identity"><span className="research-symbol-logo">{selected.symbol.slice(0, 2)}</span><span><strong>{selected.name}</strong><small>{selected.symbol} · {selected.market === 'US' ? '美股' : 'A股'} · {selected.currency}</small></span></div>
