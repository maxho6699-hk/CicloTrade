...
import { StockScreenerRoute } from './StockScreenerRoute'
...
const demoOpportunities: OpportunityPreview[] = [
  ...
  { id: 'demo-tsla-put', symbol: 'TSLA 2026-09-18 P300', underlying: 'TSLA', name: 'Tesla 看跌期权', market: 'US', kind: 'option', action: '暂不买入 · 等正股跌破', actionTone: 'neutral', contract: 'wait', priceRange: '股票跌破 312.00', quantityLine: '未触发前为 0', riskLine: '到期与 IV 风险', targetLine: '触发后计算组合退出', reason: '期权不是替代止损，先等正股触发条件。', verification: '等待条件没有发生。', hasEvidence: false, updatedAt: '11:08', demo: true },
...
export function OpportunitiesPage() {
  ...
  if (searchParams.get('tool') === 'screener') return <StockScreenerRoute />
      ...
      <div className="page opportunity-detail-page">
        ...
        <div className="opportunity-detail-layout">
          ...
          <aside className="opportunity-proof data-panel">
            ...
            {selectedOpportunity.hasEvidence ? <><p>该正式事件已在 CicloTrade 官方模拟账户产生关联成交，可以进入 K 线查看买卖标记和盈利/亏损区间。</p><button className="button primary wide" type="button" onClick={() => navigate(`/research?market=${selectedOpportunity.market}&symbol=${encodeURIComponent(selectedOpportunity.underlying)}&event_id=${selectedOpportunity.eventId ?? ''}&panel=建议`)}>检视 K 线证据 <ArrowRight size={15} /></button></> : <div className="opportunity-no-proof"><ShieldAlert size={21} /><strong>还没有可检视的 K 线证据</strong><span>只有官方模拟账户已经执行、并能关联正式事件的股票才显示证据按钮。</span></div>}
