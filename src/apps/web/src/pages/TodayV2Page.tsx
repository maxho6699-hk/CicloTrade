...
function DecisionFocus({ item, authenticated, onResearch }: { item: RecommendationItem; authenticated: boolean; onResearch: () => void }) {
  ...
  return <V2Card className="v2-focus-card">
    ...
    {item.symbol && item.market ? <RemoteMiniCandles symbol={item.symbol} authenticated={authenticated} label="首要股票 Mini K线" /> : <V2StatePanel state="locked" title="Mini K线已锁定" detail="股票代码和市场上下文必须同时存在，系统不会猜测行情。" />}
