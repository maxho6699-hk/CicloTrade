...
export function AIWorkspacePage() {
  ...
  return <div className="intelligence-page ai-workspace-page">
    ...
    {(pageState === 'ready' || pageState === 'empty') && readiness && !readiness.ready && <section className="intelligence-panel ai-readiness-compact" role="status"><div className="ai-readiness-robot"><CicloCore label="AI 服务暂不可用" size="compact" state="locked" tier={cicloTier} /></div><div className="ai-readiness-compact-copy"><span>PROVIDER READINESS</span><h2>AI 服务暂不可用</h2><p>当前没有通过服务端 readiness 核验的 provider。输入区保持安全锁定，不会生成占位回答。</p><details><summary>查看技术详情</summary><dl><div><dt>故障转移链</dt><dd>GLM → 混元 → Agnes</dd></div><div><dt>Provider 版本</dt><dd>readiness 未通过，服务端未公开</dd></div><div><dt>模型</dt><dd>readiness 未通过，服务端未公开</dd></div><div><dt>缺少字段</dt><dd>{readiness.missing.length ? readiness.missing.join('、') : '服务未通过整体核验'}</dd></div></dl><small>前端只读取服务端状态，不读取或展示任何模型密钥。</small></details></div><div className="ai-readiness-compact-actions"><button className="ai-button primary" type="button" onClick={() => void loadWorkspace()}><RotateCcw />重新检测</button><Link className="ai-button" to="/admin"><Settings2 />前往配置</Link></div></section>}
    {(pageState === 'ready' || pageState === 'empty') && <div className="ai-workspace-shell">
      <aside className="intelligence-panel ai-session-sidebar" aria-label="AI 会话列表">
        <header className="ai-sidebar-header"><div><span>CONVERSATIONS · {sessions.length}</span><h2>研究会话</h2></div><button className="ai-icon-button" type="button" aria-label="新建 AI 研究会话" title={readiness?.ready ? '新建 AI 研究会话' : 'AI 服务 readiness 未通过'} disabled={!readiness?.ready} onClick={() => setShowCreate((value) => !value)}><Plus /></button></header>
