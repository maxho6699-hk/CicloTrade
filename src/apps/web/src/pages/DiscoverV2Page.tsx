...
function CandidateRow({ item, authenticated, selected, onSelect }: { item: RecommendationItem; authenticated: boolean; selected: boolean; onSelect: () => void }) {
  return <tr className={`v2-candidate-row ${selected ? 'is-selected' : ''}`} aria-selected={selected}>
    ...
    <td><RemoteMiniCandles symbol={item.symbol} authenticated={authenticated} label={`${item.symbol || '股票'} Mini K线`} /></td>
...
export function DiscoverV2Page() {
  ...
  const research = (item: RecommendationItem | null) => { if (!item) return; navigate(`/research?symbol=${encodeURIComponent(item.symbol || '')}&event_id=${item.event_id}`) }
  const paper = (item: RecommendationItem | null) => { if (!item) return; navigate(`/paper?symbol=${encodeURIComponent(item.symbol || '')}&market=${encodeURIComponent(item.market || '')}&source=screener`) }
  const alertDraft = (item: RecommendationItem | null) => { if (!item) return; navigate(`/research?symbol=${encodeURIComponent(item.symbol || '')}&panel=预警&draft=1`) }
