...
const commissionStatusLabel = (value: ReferralPortal['commissions'][number]['status']) => localizeText(({ pending: '冻结中', withdrawable: '可提现', partially_clawed_back: '部分回退', clawed_back: '已回退' } as Record<string, string>)[value] ?? `未知佣金状态：${String(value).slice(0, 80)}`)
...
const bonusStatusLabel = (value: ReferralPortal['program']['bonus_progress']['status']) => localizeText(({ not_qualified: '尚未达到奖金门槛', pending: '奖金冻结中', earned: '奖金已获得', partially_clawed_back: '奖金部分追回', clawed_back: '奖金已追回' } as Record<string, string>)[value] ?? `未知奖金状态：${String(value).slice(0, 80)}`)
...
function StatePanel({ state, retry }: { state: Exclude<ViewState, 'ready'>; retry: () => void }) {
  ...
  return <section className={`promotion-state ${state}`} role={state === 'error' || state === 'forbidden' ? 'alert' : 'status'}>
    <div className="promotion-state-summary"><Icon /><span><strong>{localizeText(content[1])}</strong><small>{localizeText(content[2])}</small></span>{state === 'error' && <button className="button secondary" type="button" onClick={retry}><RefreshCw size={16} />{localizeText('重新读取')}</button>}</div>
    <div className="promotion-state-guide" aria-label={localizeText('推广功能边界')}>
      <article><ShieldCheck /><span><strong>{localizeText('邀请归因')}</strong><small>{localizeText('只展示服务端确认的访问、注册和首单结算。')}</small></span></article>
      <article><FileClock /><span><strong>{localizeText('佣金冻结')}</strong><small>{localizeText('冻结期、退款追回与可提现金额均由资金账本返回。')}</small></span></article>
      <article><Landmark /><span><strong>{localizeText('提现审核')}</strong><small>{localizeText('申请保持幂等，付款状态必须由管理员回执确认。')}</small></span></article>
    </div>
  </section>
}
export function PromotionCenterPage() {
  ...
  const withdrawalStatus = useMemo(() => {
    ...
    const eligibility = portal.withdrawal_eligibility
    if (eligibility.status !== 'eligible') return eligibility.status
    if (!Number.isSafeInteger(amountMinor) || amountMinor < eligibility.min_minor) return 'below_min'
    if (amountMinor > eligibility.max_minor) return 'above_max'
    if (amountMinor > eligibility.available_minor) return 'ineligible'
    return 'eligible'
  }, [amountMinor, portal])
  const withdrawalStatusText = useMemo(() => {
    if (!portal) return localizeText('提现资格暂时未知。')
    if (withdrawalStatus === 'eligible') return localizeText('当前条件满足，可提交申请；提交时服务端会再次校验。')
    if (withdrawalStatus === 'below_min') return `${localizeText('申请金额不得低于')} ${formatMinor(portal.withdrawal_eligibility.min_minor, formatLocale)}。`
    if (withdrawalStatus === 'above_max') return `${localizeText('单笔申请不得超过')} ${formatMinor(portal.withdrawal_eligibility.max_minor, formatLocale)}。`
    if (withdrawalStatus === 'ineligible') return localizeText('申请金额超过服务端返回的可提现余额。')
    return portal.withdrawal_eligibility.reason
  }, [formatLocale, portal, withdrawalStatus])
  ...
  const withdraw = async () => {
    if (!portal) return
    if (withdrawalStatus !== 'eligible') {
      setNote(withdrawalStatusText)
      return
    }
