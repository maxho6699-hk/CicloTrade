import { ArrowLeft, Braces, CheckCircle2, FileInput, GitBranch, ListChecks, LoaderCircle, RefreshCw, RotateCcw, ShieldCheck, Square } from 'lucide-react'
...
export function WorkflowTaskPage() {
  ...
  return <div className="intelligence-page workflow-page">
    ...
    {loadState === 'error' && taskId && <TruthState tone="error" title="任务读取失败" detail={error || 'Workflow 服务暂时不可用。'} action={<button className="intelligence-inline-action" type="button" onClick={() => void load()}><RotateCcw />重新读取</button>} />}
    {!taskId && (loadState === 'ready' || loadState === 'error') && <div className="workflow-index-layout">
      <section className="intelligence-panel workflow-overview" aria-labelledby="workflow-overview-title">
        <header className="workflow-title-row"><div><span>WORKFLOW ORCHESTRATION</span><h2 id="workflow-overview-title">研究任务执行图</h2><small>输入、公开任务、结构化结果与完整性校验保持可追踪。</small></div><span className={`intelligence-status ${loadState === 'ready' ? 'is-success' : 'is-warning'}`}><i />{loadState === 'ready' ? 'registry online' : 'read-only guide'}</span></header>
        {loadState === 'error' && <div className="workflow-registry-note" role="status"><RotateCcw /><div><strong>任务列表连接暂时中断</strong><span>{error || 'Workflow 服务暂时不可用。'} 流程说明保持可见，任务记录不会用演示数据填充。</span></div><button type="button" onClick={() => void load()}>重新连接</button></div>}
        <div className="workflow-overview-rail" aria-label="Workflow 标准节点">
          <article><span><FileInput /></span><div><small>01 · INPUT</small><strong>绑定研究来源</strong><p>只接收服务端授权的股票、审议或实验室上下文。</p></div></article>
          <article><span><GitBranch /></span><div><small>02 · TASK</small><strong>创建公开任务</strong><p>任务 ID、attempt、状态与取消请求可追踪。</p></div></article>
          <article><span><Braces /></span><div><small>03 · RESULT</small><strong>生成安全制品</strong><p>只展示公开 context 与 result，不暴露任意制品 URL。</p></div></article>
          <article><span><ShieldCheck /></span><div><small>04 · VERIFY</small><strong>校验完整性</strong><p>来源、context、provenance 与结果哈希分别绑定。</p></div></article>
        </div>
        <div className="workflow-overview-metrics"><article><ListChecks /><span><strong>{tasks.length}</strong><small>当前可见任务</small></span></article><article><LoaderCircle /><span><strong>{tasks.filter((item) => item.status === 'queued' || item.status === 'running').length}</strong><small>排队 / 执行中</small></span></article><article><CheckCircle2 /><span><strong>{tasks.filter((item) => TERMINAL.has(item.status)).length}</strong><small>已进入终态</small></span></article></div>
      </section>
      <section className="intelligence-panel workflow-main" aria-labelledby="workflow-index-title"><header className="workflow-title-row"><div><span>WORKFLOW REGISTRY / PUBLIC TASKS</span><h2 id="workflow-index-title">当前 Workflow 任务</h2><small>按服务端更新时间展示，不补写任务状态。</small></div><button type="button" onClick={() => void load()} disabled={Boolean(busy)}><RefreshCw />刷新列表</button></header>{tasks.length ? <div className="workflow-artifacts">{tasks.map((item) => <Link className="workflow-index-link" to={`/workflow/${encodeURIComponent(item.task_public_id)}`} key={item.task_public_id}><article><div><strong>{item.source_kind} · {item.task_public_id}</strong><small>{item.source_public_id} · {WORKFLOW_STATUS_COPY[item.status]} · {formatTime(item.updated_at)}</small></div><WorkflowStatusPill status={item.status} /></article></Link>)}</div> : <div className="workflow-empty-list"><ListChecks /><div><strong>暂无可见任务</strong><p>从牛熊审议或实验室创建任务后，真实任务会在这里出现；当前不会展示演示记录。</p></div><Link className="intelligence-inline-action" to="/lab">打开实验室</Link></div>}</section>
      <aside className="workflow-index-aside"><section className="intelligence-panel"><header className="intelligence-section-heading"><div><span>STATUS LANGUAGE</span><h2>任务状态</h2></div></header><div className="workflow-status-guide"><span><i className="is-info" />排队 / 执行中</span><span><i className="is-warning" />部分完成 / 阻断</span><span><i className="is-success" />已完成</span><span><i className="is-error" />失败 / 超时</span></div></section><section className="intelligence-panel workflow-boundary-card"><ShieldCheck /><div><strong>公开边界</strong><p>页面不展示账户归属、原始 provenance、内部思维链或任意下载地址。</p></div></section></aside>
    </div>}
