...
function decodeRow(value: unknown): StockScreenerRow | null {
  ...
  const eligible = row.action === 'buy' && row.data_state === 'fresh' && row.health === 'healthy' && paper !== null
  if (!alert || (row.paper_prefill !== null && !paper) || typeof row.actionable !== 'boolean' || (row.blocked_reason !== null && !text(row.blocked_reason, 120)) || row.actionable !== eligible || row.actionable !== (row.blocked_reason === null)) return null
...
export function paperPrefillUrl(prefill: ScreenerPaperPrefill) { return `/paper?${new URLSearchParams({ market: prefill.market, symbol: prefill.symbol, side: prefill.side, source: 'screener' })}` }
