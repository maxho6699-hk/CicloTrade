...
- 5 档订阅、月/季/年周期、订单、Paddle、PayPal、FPS、幂等回调、退款资格及权益回滚
...
- Tiger、Telegram、SMTP 接口，以及推荐奖励、续费提醒和客服/财务/研究后台
...
自动化测试覆盖认证与会话隔离、订阅支付/退款原子性和幂等、PayPal 回跳捕获校验、FPS 后台确认、推荐/分享奖励、订单风控、模拟账户账本、预警任务、8 种策略、行情指标、选股搜索及后台权限。可在本机重跑：
...
- PayPal 商户凭证、Client ID/Secret、Webhook ID，以及正式支付、退款和争议流程验证
- Tiger 账户、OpenAPI 交易授权和私钥；先完成模拟盘验收，再配置签约用户白名单、`TIGER_ENV=live` 与实盘总开关
- SMTP 发件账户与 Telegram Bot/Chat ID，并验证投递、退信和告警接收
- 美股/A 股商业实时行情、Level 2、历史期权等相应的数据许可、API 与再分发授权
- 正式域名、DNS、HTTPS、受信反向代理，以及支付/密码重设公开回调地址
- 公司主体、收款资料、客服与退款流程，以及用户协议、隐私、风险披露、投资建议边界和目标市场合规审阅
