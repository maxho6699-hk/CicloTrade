...
from core.personal_paper.service import PersonalPaperConflict, PersonalPaperRiskRejected, VerifiedQuote
...
class PersonalPaperRiskProofService:
    ...
    def issue(self, user_id: int, raw: Any) -> dict[str, Any]:
        ...
        with self.paper.database._get_connection() as connection:
            try:
                ...
                before = self.paper._account_state(connection, season)
                decision, level, state, checks, blocking, warnings = self._evaluate(
                    order, quote, before, connection=connection,
                )
                ...
                connection.execute(
                    """INSERT INTO personal_paper_risk_proofs
                       (public_id,user_id,season_id,quote_id,account_version,draft_sha256,schema_version,
                        ...
                        blocking_reasons_json,warnings_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                ...
                event = {"event_type": "ISSUED", "decision": decision, "draft_sha256": payload["draft_sha256"]}
                connection.execute(
    ...
    def verify_and_consume(self, user_id: int, order: dict[str, Any], *, connection: Any, now: datetime) -> None:
        ...
        if row["draft_sha256"] != draft_hash or row["quote_id"] != order["quote_id"] \
                or int(row["account_version"]) != order["account_version"]:
    ...
    def _evaluate(self, order: dict[str, Any], quote: VerifiedQuote, account: dict[str, Any], *, connection: Any):
        side = order["side"]
        ...
        current_exposure = abs(minor_times_quantity(price, held))
        projected = max(0, current_exposure + (notional if side in {"BUY", "SHORT"} else -min(current_exposure, notional)))
        ...
        symbols = set(positions) | {order["symbol"]}
        if not symbols.issubset(sector_map):
            ...
            for symbol, item in positions.items():
                if int(item.get("quantity_micros", 0)):
                    industry = sector_map[symbol]
                    sector_values[industry] = sector_values.get(industry, 0) + abs(minor_times_quantity(price, int(item["quantity_micros"])))
            target_industry = sector_map[order["symbol"]]
            sector_values[target_industry] = sector_values.get(target_industry, 0) + (notional if side in {"BUY", "SHORT"} else 0)
            sector_value = sector_values.get(target_industry, 0)
            sector_pct = sector_value / equity * 100 if equity else None
            sector = _check("sector_concentration", "pass" if sector_pct is not None and sector_pct <= 35 else "warn", "行业集中度", "使用冻结研究宇宙 v1 的静态 taxonomy 估算行业集中度，不等同于实时 GICS。", {"industry": target_industry, "usd": _money(sector_value), "pct": sector_pct}, {"pct": 35}, "partial" if equity else "stale")
        peak_row = connection.execute(
            "SELECT MAX(total_equity_minor) AS peak FROM personal_paper_equity_events WHERE season_id=?",
            (account["season"]["id"],),
        ).fetchone()
        peak = max(equity, int(peak_row["peak"] or 0))
        ...
        event_row = connection.execute(
            """SELECT e.* FROM earnings_event_revisions e
               WHERE e.market='US' AND e.symbol=? AND e.status='CONFIRMED'
                 ...
                 AND NOT EXISTS (SELECT 1 FROM earnings_event_revisions newer WHERE newer.supersedes_revision_id=e.id)
